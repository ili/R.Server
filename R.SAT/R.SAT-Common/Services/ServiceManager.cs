...
namespace Rsdn.SmartApp
{
	...
	public class ServiceManager : IServicePublisher, IDisposable
	{
		...
		/// </summary>
		[NotNull]
		public IServiceRegistrationCookie Publish(
		...
		/// </summary>
		public void Unpublish([NotNull] IServiceRegistrationCookie cookie)
		{
			if (cookie == null)
			...
			if (!_byCookieMap.ContainsKey(svcCookie))
				throw new ArgumentException("Supplied cookie is invalid", "cookie");
		...
		public virtual object GetService([NotNull] Type serviceType)
		{
			...
			if (!_byTypeMap.TryGetValue(serviceType, out instanceProvider))
				return GetParentService(serviceType);
		...
		{
			foreach (var handler in _byTypeMap.Values)
				if (handler.IsInstanceCreated)
					yield return handler.GetInstance();
		}

		private object GetParentService(Type serviceType)
		{
			if (_parentProviders != null)
				foreach (var sp in _parentProviders)
				{
		...
		public void Dispose()
		{
			if (_disposingNow)
				return;
			_disposingNow = true;
			using (DisposableHelper.CreateDisposable(() => _disposingNow = false))
				_byTypeMap
					.Values
