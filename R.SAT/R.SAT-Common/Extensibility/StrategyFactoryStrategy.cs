...
namespace Rsdn.SmartApp
{
	...
	public class StrategyFactoryStrategy :
		RegKeyedElementsStrategy<Type, ExtensionStrategyFactoryInfo,
			ExtensionStrategyFactoryAttribute>
	{
		...
		{
			if (!typeof (IExtensionStrategyFactory).IsAssignableFrom(
			...
			return new ExtensionStrategyFactoryInfo(context.ExtensionType);
		}
		...
		public static void RegisterAndScan(
			...
			Type[] types)
		{
			extensionManager.Scan(types, new StrategyFactoryStrategy(publisher));
			ScanWithAllFactories(publisher, extensionManager, types);
		}

		private static void ScanWithFactory(IServiceProvider provider,
