...
namespace Rsdn.SmartApp
{
	...
	public class ServiceDataManager : IServiceDataManager
	{
		...
		private static Type CreateImplType(Type ifaceType)
		{
			...
			foreach (var propertyInfo in ifaceType.GetProperties())
			{
				...
				if (propertyInfo.PropertyType.IsValueType)
					getGen.Emit(OpCodes.Unbox_Any, propertyInfo.PropertyType);
				else
					getGen.Emit(OpCodes.Castclass, propertyInfo.PropertyType);
		...
		{
			var vals = new ValsDic();
			foreach (var propertyInfo in type.GetProperties())
				if (propertyInfo.PropertyType.IsValueType)
					vals.Add(propertyInfo.Name, Activator.CreateInstance(propertyInfo.PropertyType));
				else
					vals.Add(propertyInfo.Name, null);
			return vals;
		}
